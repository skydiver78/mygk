...
local f_cmd_code  = ProtoField.uint32("upm.cmd.code", 
                      "UPM Command Code",    base.DEC, packettypes)

local f_hdr_flags   = ProtoField.uint32("upm.hdr.flags", "UPM Header Flags", base.HEX)

-- bit flag
local f_hdr_flags_comcode   = ProtoField.uint32("upm.hdr.flags.comcode",   
                            "Command code",   base.DEC, packetbool, 0x01)
local f_hdr_flags_errcode  = ProtoField.uint32("upm.hdr.flags.errcode",  
                            "Error code",  base.DEC, packetbool, 0x02)
...
function p_upm.dissector(buf, pinfo, tree)
    ...
    subtree:add(f_cmd_code, buf(0,4))   -- start to add feld
